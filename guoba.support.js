...
const password = (field, label, bottomHelpMessage) => ({
  ...
  componentProps: {
    placeholder: '建议配置',
    // 阻止浏览器密码管理器把这些 Cookie 当成登录密码自动填充到输入框
...
const videoQualityOptions = [
  ...
  option('2K', '2k'),
  option('4K', '4k'),
  option('HDR 高动态范围', 'hdr')
]

const xiaohongshuQualityOptions = [
  option('自动根据大小选择', 'adapt'),
  option('540P', '540p'),
  option('720P', '720p'),
  option('1080P', '1080p'),
...
const schemas = [
  ...
  num('app.ThemeLightHour', '浅色开始时间', 0, 23, '点', '自动模式(0)下进入浅色的时间点，默认 6'),
  num('app.ThemeDarkHour', '深色开始时间', 0, 23, '点', '自动模式(0)下进入深色的时间点，默认 18'),
