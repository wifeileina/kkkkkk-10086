...
const src = path.join(root, 'preview/douyin.html')
const out = path.join(root, 'preview/douyin-light.html')
...
try {
  ...
  await page.screenshot({ path: path.join(root, 'preview/douyin-light.png'), fullPage: true })
  console.log('saved douyin-light.png')
} finally {
  await browser.close()
}
