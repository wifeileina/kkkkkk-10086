...
export default class KuaishouData extends Base {
  ...
  async GetData (data) {
    switch (this.type) {
      case 'one_work':
      case '单个作品信息': {
        // 同一作品多群转发时复用解析结果，避免重复请求快手 API
        const dataKey = `kuaishou:${data.photoId}`
        return await runSingleFlightData(dataKey, async () => {
          ...
          const result = { VideoData, CommentData, EmojiData }
          setCachedData(dataKey, result)
          return result
        })
      }
