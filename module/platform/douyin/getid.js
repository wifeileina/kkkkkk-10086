...
export const getDouyinID = async (url, log = true) => {
  ...
  let longLink = url
  try {
    // 仅短链（v.douyin.com / v.iesdouyin.com）需要解析长链接；
    // 完整链接（www.douyin.com/video/*、live/*、modal_id=* 等）直接进入 URL 匹配，避免冗余整页请求拖慢解析启动
    if (/v\.(?:douyin|iesdouyin)\.com/i.test(url)) {
      ...
      if (longLink === url) {
        ...
        try {
          const response = await fetch(longLink, { redirect: 'follow', signal: controller.signal })
          if (response.url && response.url !== url) longLink = response.url
        } catch {
          // 兜底重定向失败则保持原链接，交由后续 URL 匹配处理
        }
        clearTimeout(timer)
      }
    }
