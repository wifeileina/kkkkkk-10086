import { Base, baseHeaders, Networks, Render, Config, Common, downloadFile, downloadVideo, Version, processImageUrl, makeForwardMsgBatched, MAX_FORWARD_MESSAGES } from '../../utils/index.js'
                ...
                if (isVideo) {
                  try {
                    ...
                    await downloadVideo(this.e, {
                      ...
                      cacheKey: `douyin:${Detail_Data?.aweme_id || Detail_Data?.id || downloadUrl}`
                  ...
                  try {
                    ...
                    // 合并转发单条上限 100 节点，超过则拆成多个合并转发依次发送
                    const forwardList = []
                    if (Version.BotName === 'Miao-Yunzai') {
                      const nodes = imageres.map(img => ({
                        user_id: 2854196310,
                        message: img
                      }))
                      for (let i = 0; i < nodes.length; i += MAX_FORWARD_MESSAGES) {
                        forwardList.push(Bot?.makeForwardMsg(nodes.slice(i, i + MAX_FORWARD_MESSAGES)))
                      }
                    } else {
                      forwardList.push(...await makeForwardMsgBatched(Bot?.[botId], imageres, '作品图片'))
                    }
                    // 如果bot不存在或群组不存在,则默认message_id为1,防止bot上线发一堆消息
                    for (const forwardMsg of forwardList) {
