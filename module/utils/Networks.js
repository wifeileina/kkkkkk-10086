...
import { isDouyinCdnUrl } from './DownloadSupport.js'
...
export class Networks {
  ...
  async getHeaders(retryCount = 0) {
    try {
      ...
      // 抖音 CDN 对 Range 探测请求返回 403，改用普通请求获取 content-length。
      // aweme.snssdk.com/aweme/v1/play 无签名直链同样对 Range 返回 403，一并跳过 Range。
      const isPlayDirectUrl = /\/aweme\/v1\/play\//.test(this.url)
      if (!isPlayDirectUrl && !isDouyinCdnUrl(this.url)) headers.Range = 'bytes=0-0'
      const response = await this.axiosInstance.get(this.url, {
        ...config,
        timeout: 3000,
        responseType: 'stream',
  ...
  async downloadStream(progressCallback, retryCount = 0, options = {}) {
    ...
    if (!isLiveStream && retryCount === 0 && Config.upload?.downloadMultiThread === true) {
      ...
      if (probe?.total >= MULTIPART_MIN_SIZE) {
        ...
        try {
          ...
          // 部分 CDN（如小红书 xhscdn 带签名直链）对每个 Range 分片返回不同 ETag，
          // 触发资源变化/分片校验失败属误判，自动回退单线程整段下载
          if (!String(error?.code || '').startsWith('MULTIPART_')) throw error
          logger.warn(`多线程下载失败(${error.code})，自动回退单线程: ${error?.message || error}`)
        }
      }
    }
    ...
    try {
      ...
      // 重试逻辑
      // 4xx(403/ERR_BAD_REQUEST 等)是客户端错误，重试同样的 URL 无意义，直接失败快速结束，避免卡住并行的其他群解析
      if (retryCount < this.maxRetries && axiosError.code !== 'ERR_BAD_REQUEST') {
