...
class Tools {
  ...
  useDarkTheme() {
    ...
    if (configTheme === 0) {
      ...
      // 自定义深浅色切换时间（默认浅色 6:00、深色 18:00），仅自动模式生效
      const lightStart = Number.isFinite(Number(Config.app.ThemeLightHour)) ? Number(Config.app.ThemeLightHour) : 6
      const darkStart = Number.isFinite(Number(Config.app.ThemeDarkHour)) ? Number(Config.app.ThemeDarkHour) : 18
      // 支持浅色开始晚于深色开始的跨午夜情况
      const isLightHour = lightStart < darkStart
        ? (hour >= lightStart && hour < darkStart)
        : (hour >= lightStart || hour < darkStart)
      dark = !isLightHour
