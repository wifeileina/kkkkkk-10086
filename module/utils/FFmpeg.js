...
export const loopVideoWithTransition = async (options) => {
  ...
  const hasSourceAudio = await hasAudioStream(inputPath)
  if (hasSourceAudio) {
    // 无 BGM 但源视频自带音轨（如小红书 live 动图原始声）：保留并垫铺原声到完整时长，避免声音丢失
    const outputDuration = transitionEnabled ? composedDuration : duration * safeLoopCount
    const audioFilter = `${filterComplex};[0:a]asetpts=PTS-STARTPTS,apad[aout]`
    const result = await ffmpeg(
      `-y ${inputArgs} -filter_complex "${audioFilter}" ` +
        `-map "[outv]" -map "[aout]" -c:v libx264 -c:a aac -b:a 192k -pix_fmt yuv420p -t ${outputDuration} "${outputPath}"`
    )
    result.status ? logger.debug(`Live Photo 效果视频生成成功: ${outputPath}`) : logger.error('Live Photo 效果视频生成失败', result)
