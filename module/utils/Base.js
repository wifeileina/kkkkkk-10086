...
// QQBot（官方 bot）会上传前自己抓取视频 URL 转 base64（裸请求、无平台特定请求头）。
// 抖音等平台直链对裸请求会返回 403，故发送前用同样方式探测该 URL 能否公开抓取；
// 抓不了则退回本地下载后上传本地文件（SDK 直接读本地文件，不经过网络，不会 403）。
const isUrlBareFetchable = async (url) => {
  try {
...
export const downloadVideo = async (e, downloadOpt, uploadOpt) => {
  ...
  if (botAdapter === 'QQBot') logger.debug(`[URL直发] 裸探测 ${downloadOpt.video_url} => ${urlFetchable ? '可抓取' : '不可抓取(回退本地)'}`)
  const urlSent = canSendRemoteVideo && urlFetchable && await sendVideoUrl(e, downloadOpt.video_url, uploadOpt)

  if (urlSent) {
    logger.mark(`视频大小 (${fileSizeInMB} MB) 已通过URL发送，跳过本地下载`)
    // URL 直发成功，后台预下载写缓存供后续同视频群复用
    if (cacheKey && !getCachedVideo(cacheKey)) {
      runSingleFlight(cacheKey, () => downloadFile(downloadOpt.video_url, buildDownloadOpt()))
        ...
        .catch(error => logger.debug(`[缓存] URL直发后预下载写缓存失败（忽略）: ${error instanceof Error ? error.message : String(error)}`))
    }
    return true
  }
  ...
  if (cacheKey) {
    ...
    if (cached) {
      ...
      logger.debug(`[缓存命中] ${cacheKey} 复用已下载文件`)
    } else {
      ...
      logger.debug(`[真实下载] ${cacheKey} 未命中缓存，已真实下载`)
    }
  } else {
...
export const downloadFile = async (videoUrl, opt) => {
  const startTime = Date.now()
  // 每个下载请求使用独立的工作文件名，避免多群并发时互相覆盖
  const extension = String(opt.title).match(/\.[a-z0-9]{1,10}$/i)?.[0] || '.mp4'
  const workTitle = createTempVideoTitle(extension)
  const { filepath, totalBytes } = await new Networks({
    url: videoUrl,
    headers: opt.headers ?? baseHeaders,
    filepath: Common.tempDri.video + workTitle,
    timeout: 30000
