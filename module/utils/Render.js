...
// git 状态结果缓存：每次渲染都查远程会拖慢出图（含 5s 超时），缓存一段时间复用
let gitStatusCache = { time: 0, html: '' }
const GIT_STATUS_TTL = 10 * 60 * 1000
...
const gitstatus = async () => {
  const now = Date.now()
  if (gitStatusCache.html && now - gitStatusCache.time < GIT_STATUS_TTL) {
    return gitStatusCache.html
  }
  const status = await checkCommitIdAndUpdateStatus()

  let html = `& <span class="name">Id</span><span class="version">${status.currentCommitId || 'unknown'}</span>`
  // 如果有错误或无法获取远程信息，只显示当前提交ID
  if (status.error || !status.remoteCommitId) {
    html = `& <span class="name">Id</span><span class="version">${status.currentCommitId || 'unknown'}</span>`
  } else if (!status.latest) {
    html = `& <span class="name">Id</span><span class="commit_id_old">${status.currentCommitId}</span> & <span class="name">新版本</span><span class="tip">${status.remoteCommitId}</span>`
  }

  gitStatusCache = { time: now, html }
  return html
}
...
export const Render = async (path, params) => {
  ...
  const data = {
    ...
    copyright: Config.app.RemoveWatermark ? '' : `<span class="name">${Version.BotName}</span><span class="version">${Version.BotVersion}</span> & <span class="name">小卡解析 ktxyAria</span><span class="version">${Version.version}</span> ${await gitstatus()}`,
    pageGotoParams: {
      waitUntil: 'load'
    },
